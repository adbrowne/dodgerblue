...
writeAndTryRead :: MonadMyDsl m => m (Maybe Int)
writeAndTryRead = do
  q <- newQueue
  writeQueue q 1
  tryReadQueue q

writeAndRead :: MonadMyDsl m => m Int
writeAndRead = do
  q <- newQueue
  writeQueue q 1
  readQueue q

writeFromAChildProcess :: MonadMyDsl m => m Int
...
myEvalIO ::
  MyDsl TQueue a
  -> IO a
myEvalIO = DodgerBlue.evalDslIO id (\(MyDslFunctions n) -> n)

myEvalTest ::
  MyDsl DodgerBlue.Testing.Queue a
  -> IO a
myEvalTest p = return $ evalState (DodgerBlue.Testing.evalDslTest (\(MyDslFunctions n) -> return n) p) DodgerBlue.Testing.emptyEvalState
