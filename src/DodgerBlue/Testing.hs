...
data ProgramState t a =
  ExternalProgramRunning (TestProgramFree t a, Int)
  | ExternalProgramComplete a
  | InternalProgramRunning (TestProgramFree t (), Int)

mkExternalProgramRunning :: TestProgramFree t a -> ProgramState t a
mkExternalProgramRunning p = ExternalProgramRunning (p, 0)

mkInternalProgramRunning :: TestProgramFree t () -> ProgramState t a
mkInternalProgramRunning p = InternalProgramRunning (p, 0)
...
stepEvalThread stepCustomCommand (InternalProgramRunning (p,idleCount)) = do
  ...
  case result of StepResultComplete () -> return $ (Nothing, Nothing)
                 StepResultContinue (n,_) -> return $ (Just (InternalProgramRunning (n, idleCount)), Nothing)
...
stepEvalThread stepCustomCommand (ExternalProgramRunning (p,idleCount)) = do
  ...
  case result of StepResultComplete a -> return $ (Just (ExternalProgramComplete a), Nothing)
                 StepResultContinue (n,_) -> return $ (Just (ExternalProgramRunning (n,idleCount)), Nothing)
...
isIdle :: ProgramState t a -> Bool
isIdle ( InternalProgramRunning (_, x)  )= x > 0
isIdle ( ExternalProgramRunning (_, x) ) = x > 0
isIdle ( ExternalProgramComplete _ ) = True

zeroIdleCount :: ProgramState t a -> ProgramState t a 
zeroIdleCount (InternalProgramRunning (x, _)) = InternalProgramRunning (x, 0)
zeroIdleCount (ExternalProgramRunning (x, _)) = ExternalProgramRunning (x, 0)
zeroIdleCount (ExternalProgramComplete a) = ExternalProgramComplete a

checkIsComplete :: [(Text,Text,ProgramState t a)] -> LoopState t a -> (LoopState t a, Bool)
checkIsComplete runnableThreads loopState@LoopState{..} =
  let
    allIdle = getAll $ foldMap (\(_,_,ps) -> All (isIdle ps)) runnableThreads
    setAllIdle = fmap zeroIdleCount
    setCooldownMode loopState = loopState { loopStateInIdleCoolDown = True }
  in
    if loopStateInIdleCoolDown then
      undefined
    else
      if allIdle then
        (False, setAllIdle . setCooldownMode $ loopState)
      else
        (False, loopState)
  ...
  where
    go loopState@LoopState{..} = do
        ...
        case runnable of
            ...
            xs ->
                let
                  (loopState', isComplete) = checkIsComplete runnable loopState
